...
void mouseClicked () {
  ...
  switch (mode) {
    ...
    case "active":                                                   // click to select where to move the piece
      mouse=mouseSquare();
      if(!checkMove(from,mouse,true,board)) return;                             // check is the target is a legal move
      to.x=mouse.x; to.y=mouse.y;                                    // remember the target location
      if ((board[from.y][from.x] % 6) === 5 &&                       // if it's a pawn ...
        (to.y%7) ===  0)                                             // ... and it reached the last line
      {
        printBoard();
